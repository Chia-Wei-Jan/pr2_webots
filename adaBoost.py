...
for train_index, test_index in kf.split(X):
    ...
    ada_reg = AdaBoostRegressor(base_estimator=None, learning_rate=0.05, loss='linear', n_estimators=50,
                                random_state=None)
    multi_ada_reg = MultiOutputRegressor(ada_reg)

    # fit the regressor with data
    multi_ada_reg.fit(X_train, Y_train)
    print("score:", multi_ada_reg.score(X_train, Y_train))

    # predict
    Y_pred = multi_ada_reg.predict(X_test)
